...
describe("migrationsDb", () => {
    ...
    describe("AWS config loading from config file", () => {
        it("should throw error if region is not defined in config file", async () => {
            jest.spyOn(config, "read").mockReturnValue(Promise.resolve([{
                region: ''
            }]));
            await expect(migrationsDb.getDdb()).rejects.toThrow(new Error('Please provide region for porfile:default'));
        });
